...
const ValueProp: React.FC = () => {
  ...
  return (
    <section id="approach" className="py-24 lg:py-48 bg-white" ref={sectionRef}>
      <div className="max-w-[1440px] mx-auto px-6 lg:px-10">
        <div className="grid lg:grid-cols-12 gap-10 lg:gap-20 mb-20 lg:mb-32 items-end">
          <div className="lg:col-span-8">
            ...
            <h2 className="text-4xl sm:text-6xl lg:text-8xl font-medium heading-h2 leading-[1.05] tracking-tight text-[#111111]">
              Where Art Meets <br className="hidden sm:block" />
              <span className="text-[#16D12E]">Asset Allocation</span>
            </h2>
          </div>
          <div className="lg:col-span-4 lg:text-right">
            <p className="text-gray-400 font-light text-lg lg:text-xl leading-relaxed max-w-sm lg:ml-auto">
              Our philosophy is rooted in the conviction that superior returns are born from disciplined research and structural rigor.
            </p>
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-[#e9eaeb] border border-[#e9eaeb] overflow-hidden" ref={gridRef}>
          {themes.map((theme) => (
            <div 
              key={theme.id} 
              className="brief-card bg-white p-8 lg:p-16 flex flex-col min-h-auto lg:min-h-[520px] group transition-all duration-700 relative cursor-default hover:bg-[#111111]"
            >
              {/* Institutional highlight bar */}
              ...
              <div className="mb-10 lg:mb-16 flex justify-between items-start">
                <div>
                  ...
                  <span className="text-[9px] font-bold tracking-[0.3em] uppercase text-gray-400 group-hover:text-white/40 transition-colors">{theme.tagline}</span>
                </div>
                <span className="text-[9px] font-mono tracking-widest text-gray-300 group-hover:text-white/20 uppercase">{theme.briefCode}</span>
              </div>
              
              <div className="flex-grow">
                <h3 className="text-3xl lg:text-4xl font-medium mb-6 lg:mb-10 text-[#111111] group-hover:text-white transition-colors duration-500 leading-tight">
                  {theme.title}
                </h3>
                <p className="text-gray-500 font-light text-base lg:text-lg leading-relaxed group-hover:text-gray-300 transition-colors duration-500">
                  {theme.description}
                </p>
              </div>

              <div className="mt-10 lg:mt-16 pt-8 lg:pt-10 border-t border-[#f0f1f2] group-hover:border-white/10 flex items-center justify-between">
                ...
                <button className="text-[10px] font-bold tracking-[0.3em] uppercase text-[#111111] group-hover:text-white transition-colors">
                  View Dossier
                </button>
              </div>
            </div>
