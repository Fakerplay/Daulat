...
const CapitalArchitecture: React.FC<Props> = ({ isDarkMode = false }) => {
  ...
  useEffect(() => {
    ...
    class Node {
      ...
      draw() {
        ctx.fillStyle = getBaseTextColor();
        ...
        ctx.strokeStyle = isDarkMode ? '#222222' : '#f0f0f0';
        ctx.beginPath();
    ...
    class CapitalFlow {
      ...
      draw(p0: any, p1: any, p2: any) {
        ...
        ctx.fillStyle = this.isAlpha && this.t > 0.7 ? alphaAccentColor : getDotColor();
    ...
    const animate = () => {
      ...
      ctx.strokeStyle = isDarkMode ? '#222222' : '#e0e0e0';
      ctx.beginPath();
  ...
  return (
    <div className="w-full h-full relative overflow-hidden bg-white dark:bg-[#050505] cursor-none transition-colors duration-500">
      ...
      <div className="absolute bottom-8 right-8 flex flex-col items-end gap-1">
        <span className="text-[7px] font-bold tracking-[0.2em] uppercase text-gray-300 dark:text-gray-600">Alpha Coefficient: 1.42</span>
